...
fn main() {
    ...
    ecs.add_entity(Some(Position { x: 0.1, y: 0.2 }), Some(Name("Nathan".to_string())), None);
    ecs.add_entity(Some(Position { x: 0.0, y: 0.0 }), Some(Name("Ronald".to_string())), None);
    ...
    if let Some((position, name, _model)) = ecs.find_entity_components(0) {
        println!("After movement, entity {:?} is at position {:?}",name, position);
    }
   if let Some((position, name, _model)) = ecs.find_entity_components(1) {
        println!("After movement, entity {:?} is at position {:?}",name, position);
    }
